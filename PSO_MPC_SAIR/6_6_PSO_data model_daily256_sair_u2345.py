...
LARGE_INT_MAX = 0.95
LARGE_INT = 1000000
save_csv_plot_path = f'.\\1.2result_PSO_max-3_init0.5 0.55_speed0.08_CNN_1616_without_sair_event_u2345_200iter_50size\\'
...
for k in range(6, 6 + Tmax):  # k：6~35
    ...
    if k == 6:
        ...
        u_opt = ctrl_vars_history[k - 7]
        u_history[k - 7] = ctrl_vars_history[k - 7]
...
for i in range(T_COUNT):
    for j in range(16):
        row = [str(i + 1)] + [j for j in ctrl_vars_history[i]] + [obj_fit_history[i]] + [obj_g_history[i]] + [j for j in
                                                                                                              u_history[
                                                                                                                  i]]
