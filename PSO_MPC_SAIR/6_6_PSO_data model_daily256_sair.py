...
LARGE_INT_MAX = 0.95
LARGE_INT = 1000000
save_csv_plot_path = f'.\\1.1result_PSO_max-3_init0.5 0.55_speed0.08_CNN_1616_without_sair_event_200iter_50size\\'
...
for k in range(6, 6 + Tmax):  # k：6~35
    ...
    if k == 6:
        ...
        # step2: Update system architecture
        u_history[k - 7] = ctrl_vars_history[ts[1] - 7]
        real_model_output_history[k - 7] = model_runner(
            flatten_and_divide(np.array(ctrl_vars_history[ts[1] - 7]) * W_16[:, :, ending + 96 + k - 8]))
        obj_fit_history[k - 7] = obj_fit_history1[ts[1] - 7]
        ...
        try:
            while True:
                ...
                for index in range(5):
                    ...
                    opt_a2[index + 1] = opt_a2[index] + 0.86 * ((E - A3 - X3 - R3) @ BA_all @ W_16[:, :, ending + 96 + k - 7] @ np.array(opt_a2[index]).T + (E - A3 - X3 - R3) @ BX_all @ W_16[:, :, ending + 96 + k - 7] @ np.array(opt_x2[index])).T - np.diag(opt_a2[index]) @ epsilon - np.diag(opt_a2[index]) @ r_a
                    opt_x2[index + 1] = opt_x2[index] + 0.14 * ((E - A3 - X3 - R3) @ BA_all @ W_16[:, :, ending + 96 + k - 7] @ opt_a2[index] + (E - A3 - X3 - R3) @ BX_all @ W_16[:, :, ending + 96 + k - 7] @ opt_x2[index]) + np.diag(opt_a2[index]) @ epsilon - np.diag(opt_x2[index]) @ r_s
                ...
                if len(ctl_vars) == 3:
                    ...
                    u_history_temp = [u_history_temp1, u_history_temp2, u_history_temp3, u_history_temp4, u_history_temp5]
...
np.savetxt(save_csv_plot_path + f'6_PSO_data_model_ir2_afterwards_max{daily_resource}.csv', ir2_afterwards,delimiter=',')
np.savetxt(save_csv_plot_path + f'6_PSO_data_model_real_output_max{daily_resource}.csv', real_model_output_history,delimiter=',')
